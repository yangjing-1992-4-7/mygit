...
class Category extends Adminbase
{
    ...
    {   
        ...
        if ($this->request->isAjax()) {
            ...
            $tree = new \util\Tree();
            $categorys = array();
            $result = Db::name('category')->field('category.listorder,category.modelid,category.id,category.url,category.parentid,category.catname,category.catdir,category.type,category.status,attachment.path')->alias('category')->leftJoin('attachment','category.image = attachment.id')->order(array('category.listorder', 'category.id' => 'ASC'))->select();
    ...
    public function delete()
    {
        ...
        $this->repair();
        $this->cache();
        $this->success("栏目删除成功！", url('cms/category/index'));
    }
