...
class Form
{
     ...
     * @param int $watermark_setting  水印 0不加水印，''按后台设置
     */
    public static function images($name, $id = '', $value = '', $multiple = 'false', $ext = '', $size = 0, $watermark = 0)
    {
        ...
        if (!empty($value)) {
        	...
        	foreach($value_list as $k=>$v){
        		$path = get_file_path($v) ? get_file_path($v) : $static_url . "/admin/img/none.png";
                $string .= "<div class='file-item thumbnail'><img data-original='{$path}' src='{$path}' width='100' style='max-height: 100px;'><i class='iconfont icon-delete_fill remove-picture' data-id='{$v}'></i></div>";
        	}
            
        }
        $string .= "</div><input type='hidden' name='{$name}' data-multiple='{$multiple}' data-watermark='{$watermark}' data-thumb='' data-size='{$size}' data-ext='{$ext}' id='{$id}' value='{$value}'><div class='layui-clear'></div><div id='picker_{$name}'>上传图片</div>";
        return $string;
    }
