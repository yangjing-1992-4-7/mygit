    ...
    public function index()
    {
        if ($this->request->isAjax()) {
            ...
            $data = Db::name("adminlog")
                ->alias('adminlog')
                ->page($page, $limit)
                ->order('adminlog.id', 'desc')
                ->withAttr('adminlog.create_time', function ($value, $data) {
                    return date('Y-m-d H:i:s', $value);
                })
                ->withAttr('adminlog.ip', function ($value, $data) {
                    return long2ip($value);
                })
                ->leftjoin('admin','admin.id=adminlog.uid')
	            ->field('adminlog.*,admin.nickname')
                ->select();
