...
class Menu extends Model
{
    ...
    final public function adminMenu($parentid, $with_self = false)
    {
        ...
        $result = $this->where(array('parentid' => $parentid, 'status' => 1,'is_top'=>1))->order('listorder ASC,id ASC')->cache(60)->select()->toArray();
        if (empty($result)) {
            $result = array();
        }
        if ($with_self) {
    ...
    final public function returnNodes($tree = true)
    {
        ...
        //去除status为0的菜单及其子菜单
        $ids=array();
    	$id = $this->where('status',0)->column('id');
    	$ids=array_merge($ids, $id);
    	if(count($id)>0){
    		do{
			    $id =$this->where('parentid','in',$id)->column('id');
			    if(count($id)>0){
			    	$ids=array_merge($ids, $id);
			    }
        		$num=count($id);
        ...
        if ((int) $tree) {
        	if(count($ids)>0){
        		$list = $this->where('id','notin',$ids)->order('listorder ASC,id ASC')->select()->toArray();
        	}else{
        	...
        	if(count($ids)>0){
        		$nodes = $this->where('id','notin',$ids)->order('listorder ASC,id ASC')->select()->toArray();
        	}else{
