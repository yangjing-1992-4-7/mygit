...
class Group extends Adminbase
{
    ...
    public function index()
    {
        if ($this->request->isAjax()) {
            $_list = $this->Member_Group->alias('member_group')
			->leftjoin('attachment','member_group.icon=attachment.id')			
	        ->field('member_group.*,attachment.path')->order(["issystem" => "DESC", "id" => "ASC"])->select();
    ...
    public function role_edit(){
        ...
        if($this->Member_Group->where('id',$data['id'])->update($date)){
        	$result = array("code" => 'success', "data" => $value);           
        }else{
        	$result = array("code" => 'error', "data" => $value);
        }
        return json($result);
    }
